...
class login:
	...
	def __login__(self):
		os.system('clear')
		banner()
		cokie = input(' [%s*%s] cookie : '%(O,N))
		if cokie in ['']:
			time.sleep(2);login().__login__()
		else:
			try:
				cc = requests.get('https://business.facebook.com/business_locatio=%s'%(cokie)).json()['name']
				open('cokie.x','w').write(token)
	...
	def bot(self):
		...
		try:
			cokie = open('cokie.x','w').read()
		except IOError:
			jalan('\n [%s!%s] cokie mokad ganti akun!'%(M,N));time.sleep(1);login().__login__()
...
class menu:
	...
	def main(self):
		os.system('clear')
		try:
			cok = open('cokie.x','w').read()
		except IOError:
			print(' [%s+%s] Kamu belum login'%(M,N));login().__login__()
		try:
			r = requests.get('https://business.facebook.com/business_locatio=&s'%(toke)).json()['name']
		except KeyError:
			print(' [%s!%s] Login gagal ...'%(M,N));os.system('rm -rf cokie.x');time.sleep(2);login().__login__()
		...
		jalan(' %s[ %sselamat datang tod %s%s ]'%(N,H,r,N))
		print(' %s[%s•%s] YOUR IP  : %s'%(N,O,N,IP))
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			token = open('token.x','r').read()
			try:
				pler = int(input(' %s[%s•%s] berapa target ? : '%(N,O,N)))
			except:pler = 1
			for ikeh in range(pler):
				ikeh += 1
				try:
					print(' %s'%(N))
					idt = input(' %s[%s•%s] idz ke %s : '%(N,O,N,ikeh))
...
class crack:
	...
	def free(self):
		...
		with MALIK XD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
					sempak = name.split(' ')
